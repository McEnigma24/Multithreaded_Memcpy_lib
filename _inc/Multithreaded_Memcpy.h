...
#include "on_write_allocate_memcpy.hpp"
#include <omp.h>
...
namespace Multithreaded_Memcpy
{
    ...
    void cpy(char* dest, char* src, const size_t byte_size, const int thread_count)
    {
        ...
        #pragma omp parallel
        {
            ...
            std::memcpy(dest_tab[i], src_tab[i], local_chunk_byte_size);
        }
        // clang-format on
    }

    void cpy_no_cache_polution(char* dest, char* src, const size_t byte_size, const int thread_count)
    {
        cpy_checks(dest, src, byte_size, thread_count);
        ...
        #pragma omp parallel
        {
            ...
            no_write_allocate_memcpy(dest_tab[i], src_tab[i], local_chunk_byte_size);
        }
        // clang-format on
    }

    u64 check_how_many_threads_get_fastest_time_std_memcpy(const size_t byte_size)
    {
        Nano_Timer::Timer t;
        u64 smallest_time = -1;
    ...
    u64 check_how_many_threads_get_fastest_time_simd_stream_memcpy(const size_t byte_size)
    {
        Nano_Timer::Timer t;
        u64 smallest_time = -1;
        u64 smallest_thread_num = 0;
        ...
        for (int i = 1; i <= 18; i++)
        {
            ...
            for (int reps = 0; reps < all_reps; reps++)
            {
                t.start();
                {
                    cpy_no_cache_polution(dest, src, byte_size, i);
                }
                t.stop();
            }
